...
export const createLengthExtraData = (
	lengthExtraData: LengthExtraData,
): LengthExtraData => {
	return {
...
export const validate = (
	formData: FormData,
	validationData: ValidationData<ExtraData>[],
): boolean => {
	let flag = true;
	for (let i = 0; i < validationData.length; i++) {
		...
		formData.forEach((value, key) => {
			if (
				key === validationData[i].key &&
				validationData[i].rule == 'length'
			) {
				if (
					(<string> value).length <
						(<LengthExtraData> validationData[
							i
						].extra).min ||
					(<string> value).length >
						(<LengthExtraData> validationData[
